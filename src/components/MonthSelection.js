...
function MonthSelection(props) {
	...
	const {monthOption, selectedMonthDate, selectedMonthDay, selectedMonthDayOrder} = state;

	const handleMonthOptionChange = event => {
		setState({monthOption: event?.target?.value});
	};
	const handleMonthDateChange = event => {
		setState({selectedMonthDate: event?.target?.value});
	};
	const handleMonthDayChange = event => {
		setState({selectedMonthDay: event?.target?.value});
	};
	const handleMonthDayOrderChange = event => {
		setState({selectedMonthDayOrder: event?.target?.value});
	};
				...
				<div className={css.customContainer}>
					...
					>
						{weekDaysOptions().map(item => {
							const optionDisable =
								item.name === 'Weekday' && !['First', 'Last'].includes(selectedMonthDayOrder);
							return (
								<option key={item.name} disabled={optionDisable} value={item.name}>
									{item.name}
								</option>
							);
						})}
